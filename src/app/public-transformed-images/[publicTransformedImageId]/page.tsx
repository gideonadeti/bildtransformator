...
const Page = () => {
  const params = useParams<{ publicTransformedImageId: string }>();
  const router = useRouter();
  const publicTransformedImageId = params.publicTransformedImageId;
  const {
  ...
  } = useTransformedImage(publicTransformedImageId);
  const { user } = useUser();
  const [isTransformDialogOpen, setIsTransformDialogOpen] = useState(false);
  const [isDeleteDialogOpen, setIsDeleteDialogOpen] = useState(false);

  const transformedImage = publicTransformedImageQuery.data;
  const isLoading = publicTransformedImageQuery.isPending;
  ...
  const isOwner = user?.id === transformedImage?.originalImage.userId;
        ...
        <div className="space-y-4">
          ...
          <div className="py-12 text-center">
            <h1 className="text-2xl font-bold mb-2">
              Public Transformed Image Not Found
            </h1>
            <p className="text-muted-foreground">
              The public transformed image you're looking for doesn't exist.
            </p>
          </div>
        </div>
      </div>
                  ...
                  {displayedTransformedTransformedImages.map(
                    (transformedTransformedImage) => (
                      <TransformedImageCard
                        ...
                        originalImageName={
                          transformedImage.originalImage.originalName
                        }
                      />
                    )
                  )}
