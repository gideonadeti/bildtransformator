...
const Page = () => {
  const params = useParams<{ publicImageId: string }>();
  const publicImageId = params.publicImageId;
  const { user } = useUser();
  const {
    publicImagesQuery,
  ...
  const isLoading = publicImagesQuery.isPending;

  const image = useMemo(() => {
    return images.find((img) => img.id === publicImageId) || null;
  }, [images, publicImageId]);
        ...
        <div className="space-y-4">
          ...
          <div className="py-12 text-center">
            ...
            <p className="text-muted-foreground">
              The image you're looking for doesn't exist or is not public.
            </p>
          </div>
        </div>
      </div>
